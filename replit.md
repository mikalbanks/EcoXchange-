...
- **Real-Data Backtest Mode**: Backtest engine supports `meterDataSource: "stored"` to load real production data from energy_production table (filtered to backtest window) instead of synthetic PVDAQ. Route `/api/backtest/run` accepts `projectId` + `meterDataSource` params.
