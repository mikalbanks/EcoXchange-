...
- **SCADA Operations Page**: Admin/Developer-accessible /operations route with tabbed interface for SCADA data management. Four tabs: Data Sources (shows all project SCADA sources with quality/status badges, fetched from /api/operations/data-sources), CSV Upload (stub with drag-drop zone and simulated field mapping preview), Connectors (displays available SCADA providers — AlsoEnergy, Enphase, SolarEdge, NREL PVDAQ, Power Factors — with connect/coming-soon actions), Reconciliation Log (chronological demo ingestion events with quality check badges). Includes "Publish Updated Metrics" action that simulates cache refresh. All sections marked as demo/prototype. Route protected for ADMIN and DEVELOPER roles. API: GET /api/operations/data-sources (role-aware, returns project metadata + sources).
