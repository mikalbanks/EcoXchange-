// The dashboard's single door onto the benchmark artifact.
//
// WHY THIS FILE DOES NOT IMPORT FROM ../../../shared/benchmark
//
// It used to. The Cloudflare Workers build for `ecoxchange-demo` — the one
// that serves demo.ecoxchange.net via the build shim in ../ecoxchange-demo —
// went red the moment this package reached outside its own directory, while
// the root builds (www) reaching `shared/` stayed green. A full checkout
// builds fine either way, so the difference is what that build can see.
//
// So the rule for this package is: never read a file outside
// ecoxchange-dashboard/. `shared/benchmark/` remains the canonical artifact
// and serves client/ (which builds from the repo root); this package keeps a
// committed copy, and shared/benchmark/benchmark.test.ts fails the root suite
// if the two ever drift apart — including the copy served at
// public/benchmark-results.json.
//
// Everything below is imported from here and nowhere else. No component may
// re-type a benchmark statistic or re-implement the bucket lookup: a figure
// that appears on the benchmark page and in the exported PDF has to be the
// same figure, and this module is what makes that true.
//
