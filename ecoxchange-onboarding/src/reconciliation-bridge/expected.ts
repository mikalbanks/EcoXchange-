// Source of truth for the physics model is the reconciliation engine at
// /ecoxchange-reconciliation-engine/src/physics/. This file holds a faithful,
// minimal copy so the onboarding service can run without a workspace link.
// Keep in sync with that package; any divergence is a bug.
...
export function calculateExpectedGeneration(
  ...
  const commissioning = new Date(input.commissioning_date + "T00:00:00Z");
  ...
  const daily = input.daily_irradiance.map((d) => {
    const poa = transposeDay(d, input.latitude, input.tilt_deg, input.azimuth_deg);
  ...
  return {
    ...
    assumptions: {
      ...
      system_losses: input.system_losses,
      albedo: ALBEDO,
      transposition_model: "hay_davies",
    },
  };
