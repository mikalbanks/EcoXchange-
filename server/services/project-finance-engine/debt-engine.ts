...
export type DebtFinancingSummary = Omit<FinancingSummary, "lender_fee" | "dsra">;
export type DebtReconciliationResult = Pick<
  ReconciliationResult,
  "debt_reconciliation_difference" | "debt_reconciled"
>;
...
export interface DebtEngineResult {
  ...
  financing_summary: DebtFinancingSummary;
  reconciliation: DebtReconciliationResult;
...
function checkScheduleFeasibility(
  ...
  for (let index = 0; index < debtServiceProfile.length; index += 1) {
    if (balance <= MONEY_TOLERANCE) {
      return { feasible: true, ending_balance: 0, negative_amortization_year: null };
    }
...
export function solveMaximumFeasibleDebt(
  ...
  const solvedCheck = checkScheduleFeasibility(low, annualInterestRate, debtServiceProfile);
...
export function calculateDebtSizing(
  ...
  return {
    ...
    schedule_scale: feasibleDscrDebt > 0 ? permanentDebt / feasibleDscrDebt : 0,
  };
}
...
export function calculateDebtEngine(
  ...
  const financingSummary: DebtFinancingSummary = {
  ...
  const reconciliation: DebtReconciliationResult = {
