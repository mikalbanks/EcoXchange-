...
export function registerProjectFinanceV1Routes(app: Express,deps:ProjectFinanceApiDeps={}) {
  ...
  app.post(`${API}/scenarios/:scenarioId/calculate`,handler(async(req,res,ctx)=>{const scenarioId=parseId(req.params.scenarioId),body=calculateBodySchema.parse(req.body??{}),scenario=await service.getScenario(ctx,scenarioId);const result=await service.calculations.calculateScenario({context:ctx,projectId:scenario.project_id,scenarioId,...selector(body),idempotencyKey:parseIdempotency(req)});return ok(res,calculationDto(result))}));
  app.post(`${API}/calculation-runs/:runId/underwrite`,handler(async(req,res,ctx)=>{const runId=parseId(req.params.runId),body=underwriteBodySchema.parse(req.body??{}),calc=await service.calculations.getCalculationRun(ctx,runId);const result=await service.underwriting.underwriteCalculation({context:ctx,projectId:calc.run.project_id,scenarioId:calc.run.scenario_id,calculationRunId:runId,...selector(body),idempotencyKey:parseIdempotency(req)});return ok(res,underwritingDto(result))}));
  ...
  app.get(`${API}/underwriting-runs/:runId`,handler(async(req,res,ctx)=>ok(res,underwritingDto(await service.getUnderwritingRun(ctx,parseId(req.params.runId))))));
