...
export default function ProjectFinanceProjects() {
  ...
  return <DashboardLayout title="Project Finance" description="Guided sponsor-side project and underwriting workspace." actions={<Button asChild><Link href="/developer/project-finance/projects/new"><Plus className="mr-2 h-4 w-4"/>Create Project</Link></Button>}>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {active.map(p=><Card key={p.id} className="flex flex-col"><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>{p.name}</CardTitle><CardDescription>{humanize(p.technology)} · {p.state_code||"State not set"}</CardDescription></div><Badge variant="outline">{humanize(p.development_status)}</Badge></div></CardHeader><CardContent className="flex flex-1 flex-col gap-4"><div className="grid grid-cols-2 gap-3 text-sm"><div><div className="text-muted-foreground">Capacity</div><div className="font-medium">{capacity(p)}</div></div><div><div className="text-muted-foreground">Latest scenario</div><div className="font-medium">{p.latest_scenario_name||"None"}</div></div><div><div className="text-muted-foreground">Scenario state</div><div className="font-medium">{humanize(p.latest_scenario_status)}</div></div><div><div className="text-muted-foreground">Latest underwriting</div><div className="font-medium">{humanize(p.latest_underwriting_status)||"None"}</div></div></div><div className="mt-auto flex gap-2"><Button className="flex-1" asChild><Link href={`/developer/project-finance/projects/${p.id}`}><FolderKanban className="mr-2 h-4 w-4"/>Open Project</Link></Button><Button variant="outline" size="icon" aria-label={`Archive ${p.name}`} disabled={archive.isPending} onClick={()=>archive.mutate(p.id)}><Archive className="h-4 w-4"/></Button></div></CardContent></Card>)}
    </div>
  </DashboardLayout>;
}
