...
function InputField({ label, value, onChange, suffix, step = "any" }: { label: string; value: number; onChange: (value: number) => void; suffix?: string; step?: string }) {
  return <div className="space-y-1.5"><Label>{label}</Label><div className="relative"><Input type="number" value={Number.isFinite(value) ? value : ""} step={step} onChange={(event) => onChange(event.target.value === "" ? Number.NaN : Number(event.target.value))} className={suffix ? "pr-16" : undefined} />{suffix && <span className="absolute right-3 top-2.5 text-xs text-muted-foreground">{suffix}</span>}</div></div>;
}
...
export default function BankabilityWorkspace() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/project-finance/defaults?capacityMwAc=${encodeURIComponent(capacity)}`, { credentials: "include" })
      ...
      .then((defaults) => {
        ...
        const isReferenceFiveMw = Math.abs(capacity - 5) < 1e-9;
        const next: FinanceInput = {
          projectName: selectedProject.name,
          capacityMwAc: capacity,
          p50CapacityFactor: isReferenceFiveMw ? 0.24 : Number.NaN,
          annualDegradationRate: 0.005,
          projectLifeYears: 25,
          ppaTermYears: 25,
          yearOnePpaPricePerMwh: isReferenceFiveMw ? 55 : Number.NaN,
          annualPpaEscalationRate: 0.01,
          totalProjectCapexUsd: Number.isFinite(storedCapex) && storedCapex > 0 ? storedCapex : isReferenceFiveMw ? 8_000_000 : Number.NaN,
          capexIncludesContingency: true,
          yearOneOpexUsd: isReferenceFiveMw ? 150_000 : Number.NaN,
          annualOpexEscalationRate: 0.025,
          itcRate: defaults.assumptions.itcRate,
          itcEligibleBasisPercent: isReferenceFiveMw ? 0.95 : Number.NaN,
  ...
  >
    <div className="space-y-6">
      ...
      {!selectedProjectId && <Card><CardContent className="p-8 text-center"><Banknote className="mx-auto mb-3 h-9 w-9 text-muted-foreground" /><h2 className="font-serif text-xl font-semibold">Select a project to begin</h2><p className="mt-1 text-sm text-muted-foreground">Existing project name, technology, size, stage, and stored capex prepopulate where available.</p></CardContent></Card>}

      {input && <>
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium">{input.projectName} · {input.capacityMwAc} MW · {labelize(selectedProject?.technology || "SOLAR_PV")}</p><p className="text-xs text-muted-foreground">Scenario: {scenario === "BASE" ? "Base Case" : "Custom Scenario"}</p></div><div className="flex gap-2"><Button variant="outline" onClick={resetBase} disabled={scenario === "BASE"}><RefreshCw className="mr-2 h-4 w-4" />Reset Base Case</Button><Button onClick={analyze} disabled={isAnalyzing}>{isAnalyzing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Banknote className="mr-2 h-4 w-4" />}Analyze Financing</Button></div></div>

        <Card><CardHeader><CardTitle>Scenario assumptions</CardTitle><p className="text-sm text-muted-foreground">Stored project facts are reused where available. Debt-policy values come from the backend. A 5 MW project may prefill the published reference-case operating assumptions; other missing project economics remain blank until entered.</p></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
