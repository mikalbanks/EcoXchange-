    ...
    <DashboardLayout
      ...
      actions={
        <div className="flex flex-wrap gap-2">
          <Link href="/developer/finance">
            <Button variant="outline" className="gap-2" data-testid="button-bankability-cta">
              <Banknote className="h-4 w-4" />
              Bankability & Sponsor Equity
            </Button>
          </Link>
      ...
      <Card className="mb-6 border-yellow-500/30 bg-yellow-500/10">
        <CardContent className="p-4 flex items-center gap-4">
          ...
          <p className="text-sm text-yellow-500" data-testid="text-disclaimer-banner">
            Project-finance results are indicative only and are not a financing commitment, credit decision, lender approval, tax opinion, or legal advice.
          </p>
        </CardContent>
      </Card>
        ...
        <CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="flex items-center gap-2 font-semibold">
              ...
              Estimate bank debt capacity and sponsor equity
            </h3>
            <p className="text-sm text-muted-foreground">
              Run lender-style project-finance analysis against an existing project, then change PPA, capex, debt, and tax-credit assumptions in a custom scenario.
            </p>
          </div>
          <Link href="/developer/finance">
            <Button className="gap-2 shrink-0" data-testid="button-finance-workspace-cta">
              Analyze Financing
        ...
        <CardContent>
          {projectsLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <Link href="/developer/onboard"><Button size="sm" className="gap-2"><Plus className="h-4 w-4" />Onboard Your First Project</Button></Link>
            </div>
          ) : (
                ...
                <div key={project.id} className="flex flex-col gap-3 rounded-lg border border-border p-4 sm:flex-row sm:items-center sm:justify-between">
                  <Link href={`/developer/projects/${project.id}`} className="flex-1 min-w-0 hover:text-primary" data-testid={`card-project-${project.id}`}>
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      {project.readinessScore && <StatusBadge status={project.readinessScore.rating} type="readiness" />}
                    </div>
                    <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                      <span>{project.technology.replace("_", " ")}</span><span>{project.state}, {project.county}</span>{project.capacityMW && <span>{project.capacityMW} MW</span>}
                    </div>
                  </Link>
                  <div className="flex flex-wrap items-center gap-2 shrink-0">
                    {project.missingCount > 0 && <span className="flex items-center gap-1 text-xs text-yellow-500"><AlertTriangle className="h-3.5 w-3.5" />{project.missingCount} missing</span>}
